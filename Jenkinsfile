...
pipeline {
    ...
    environment {
        ...
        VAULT_TOKEN    = sh (
            script: 'vault auth -method=ldap username=${env.CREDENTIALS_VAULT_USER} password=${env.CREDENTIALS_VAULT_PASSWORD} 2>&1 > /dev/null && vault read -field id auth/token/lookup-self', returnStdout: true
        ).trim()
    }
